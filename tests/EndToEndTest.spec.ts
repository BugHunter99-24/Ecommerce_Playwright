...
async function performRegistration(page: Page): Promise<string> {
    ...
    await registrationPage.setPolicy();  // Accept the privacy policy
    await registrationPage.continue();     // Submit the registration form
...
async function performLogin(page: Page, email: string) {
    ...
    await loginPage.loginprocess(email, "test123");  // Use the registered credentials
